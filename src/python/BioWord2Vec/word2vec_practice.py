...
res_dir = "../res/"
Windows_res_dir = '/media/sunkyu/327680A376806985/workspace/'
result_dir = "../result/"
...
pubmed_sentence_dir = '/'.join([Windows_res_dir,'pubmed_sentences'])
...
if __name__=='__main__':
    # import time
    # sentence_set = get_pubmed_abstract()
    # print len(sentence_set)

    model = train_word2vec()

    # possible to resume training
    # model.save(res_dir + "word2vec/temp_model_160526")
    model.save(res_dir + "word2vec/temp_model_160906")

    # not possible to resume training with this model file
    model.save_word2vec_format(res_dir + "word2vec/pubmed_bionlp_300.bin", binary=True)
##########################################################################################
    # s = time.time()
    # model = word2vec.Word2Vec.load_word2vec_format(
    #     res_dir + "word2vec/fixed_model_160526.bin", binary=True)
    # f = time.time()
    # print f - s
